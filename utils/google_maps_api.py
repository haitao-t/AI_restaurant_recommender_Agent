...
if not GMAPS_API_KEY:
    logging.warning("GOOGLE_MAPS_API_KEY not found in environment variables. Real API calls will fail.")
    gmaps = None
else:
    try:
        gmaps = googlemaps.Client(key=GMAPS_API_KEY)
    except Exception as e:
        logging.error(f"Failed to initialize Google Maps client: {e}", exc_info=True)
        gmaps = None
