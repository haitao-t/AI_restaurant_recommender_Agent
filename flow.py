...
from nodes import (
    ...
    DecideActionNode,
    AnalyzeReviewsBatchNode, CalculateFitScoreNode, GenerateResponseNode,
...
def create_recommendation_flow():
    ...
    analyze_reviews = AnalyzeReviewsBatchNode() # Retries configured in Node class
    ...
    # 3. Define branches based on decision logic
...
if __name__ == '__main__':
    # Example of how to create the flow (usually called from main.py)
    print("Testing flow creation...")
    flow = create_recommendation_flow()
    print(f"Flow created. Start node: {flow.start.__class__.__name__}")
    # You would typically run the flow in main.py, not here
    # Example: flow.run(shared_data)
    print("Flow definition seems okay.") 